            ...
            <template>
                ...
                >
                    ...
                    <template v-slot:item.now="{ item }">
                        <v-chip :color="getColor(item.limit, item.now)" dark
...
export default class Planning extends Vue {
    ...
    private valid = false;
    private titleRules = [(v: string) => !!v || "Titre requis"];
    private limitRules = [
        (v: number) => !!v || "Temps requis",
        (v: number) => v > 0 || "Temps invalide",
    ];
    private relatedRules = [(v: CustomItem) => !!v || "Item requis"];
            ...
            .then(async (items) => {
                this.schedules = await Promise.all(
                    items.map(async (i) => {
                        ...
                        const totalCurrent = await ScheduleHelpers.foundTotalTimes(
                            firstDayCurrent,
                            lastDayCurrent,
                            i
                        );
                        const totalPrevious = await ScheduleHelpers.foundTotalTimes(
                            firstDayPrevious,
                            firstDayCurrent,
                            i
                        );
    ...
    public async editItem(item: CustomSchedule) {
        ...
        this.editedItem.limit = parseFloat(
            (this.editedItem.limit / 3600).toFixed(1)
        );

        if (this.editedItem.obj) {
            const typeObj = (await this.editedItem.obj.getRelated())
                //@ts-ignore
                .className();
            //@ts-ignore
            const idObj = (await this.editedItem.obj.getRelated()).id;
            this.allItems.forEach((i) => {
                if (i.type == typeObj && i.id == idObj) this.selectedAll = i;
            });
        }
        this.dialog = true;
    }
